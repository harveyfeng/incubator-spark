...
// more info on how we are requesting containers.
private[yarn] class YarnAllocationHandler(
  ...
  // TODO(harvey): Confirm which "three" exactly
  ...
  // Note: there are no concurrent hash sets, so the value for each entry is always true.
  ...
  def allocateContainers() {
    ...
    if (allocatedContainers.size > 0) {
      ...
      // TODO(harvey): Cleanup this group-by...
      ...
      for (candidateHost <- hostToContainers.keySet) {
        ...
        // TODO(harvey): Convert to this format:
        // remainingContainersOpt = remainingContainersOpt.map ( ... )
        ...
        if (remainingContainers != null) {
          ...
          if (rack != null) {
            ...
            if (requiredRackCount >= remainingContainers.size) {
              ...
              // TODO(harvey): Does this have an effect?
      ...
      logDebug("""
        Finished allocating %s containers (from %s))
        Current number of workers running: %d
        pendingReleaseContainers: %s
        """.format(
          allocatedContainersToProcess, allocatedContainers,
          numWorkersRunning.get(),
          pendingReleaseContainers))
    }
    ...
    if (completedContainers.size > 0) {
      ...
      for (completedContainer <- completedContainers) {
        ...
        if (pendingReleaseContainers.containsKey(containerId)) {
          ...
          // Decrement the number of workers running. The next iteration of ReporterThread will
          // take care of allocating.
          // TODO(harvey): Figure out what this means...
          numWorkersRunning.decrementAndGet()
          logInfo("Completed container %d (state: %s, http address: %s)".format(
            containerId,
  ...
  def createRackResourceRequests(
    ...
    for ((rack, count) <- rackToCounts) {
      requestedContainers ++= createContainerRequests(
        AllocationType.RACK,
        rack,
        count,
  ...
  // TODO(harvey): Rename to 'requestContainers()'.
  def addContainerRequests(numWorkers: Int) {
    val containerRequests: List[ContainerRequest] =
      if (numWorkers <= 0 || preferredHostToCount.isEmpty) {
        ...
        for ((candidateHost, candidateCount) <- preferredHostToCount) {
          ...
          if (requiredCount > 0) {
            hostContainerRequests ++= createContainerRequests(
              AllocationType.HOST,
              candidateHost,
              requiredCount,
              YarnAllocationHandler.PRIORITY)
        ...
        val anyContainerRequests = createContainerRequests(
          AllocationType.ANY,
          resource = null,
          numWorkers,
          YarnAllocationHandler.PRIORITY)
  ...
  private def createContainerRequests(
